...
Original file is located at
    https://colab.research.google.com/drive/15DzTQvzsLmxCC_9aJgzIz2ze4X4NTYaU
"""
...
def approx_ckm_stage(entries, ontology):
    ...
    if has_cvd:
        stage = 4
        stage_label = "Stage 4: Clinical CVD in CKM"
        used = ["CLINICAL_CVD"]
    elif has_kidney_failure:
        stage = 3
        stage_label = "Stage 3: Subclinical CVD in CKM"
        used = ["VERY_HIGH_CKD"]
    elif has_metabolic:
        stage = 2
        stage_label = "Stage 2: Metabolic risk factors, CKD, or both"
        used = ["STAGE_2_METABOLIC_CKD"]
    elif has_adiposity:
        stage = 1
        stage_label = "Stage 1: Excess or dysfunctional Adiposity"
        used = ["STAGE_1_ADIPOSITY"]
    else:
        stage = 0
        stage_label = "Provisionally Stage 0 and unclassifiable"
        used = []

    return stage, stage_label, [h for n in used for h in hits[n]]

# --------------------------------------------------------------------------- #
# JSON Processing Execution Block
# --------------------------------------------------------------------------- #
if __name__ == "__main__":
    input_file = "ckm_pts_segments_10.json"
    output_file = "ckm_pts_segments_10_staged.json"
    ...
    # Correctly access the list of patients from the loaded data
    patients = all_data.get('patients', [])
    ...
    for patient in patients:
        ...
        stage_result, stage_result_label, reasons = approx_ckm_stage(entries_list, local_ontology)

        patient["ckm_stage"] = stage_result
        patient["ckm_stage_label"] = stage_result_label
