...
STAGING_PRIMER = """CKM staging (Ndumele et al. 2026, Table 4):
Stage 0 - No CKM risk factors: normal BMI/waist circumference, normoglycemia,
  normotension, normal lipids, no CKD and no subclinical or clinical CVD.
Stage 1 - Excess or dysfunctional adiposity, without other metabolic risk
  factors or CKD.
Stage 2 - Metabolic risk factors (hypertriglyceridemia, hypertension, diabetes,
  metabolic syndrome) or CKD.
Stage 3 - Subclinical CVD in CKM syndrome, or risk equivalents (e.g. very high
  predicted CVD risk, high-risk CKD).
Stage 4 - Clinical CVD (CHD, HF, stroke, PAD, AF) in a person with excess or
  dysfunctional adiposity, other CKM risk factors, or CKD.
Higher stages indicate greater absolute cardiovascular risk."""
...
def answer(question, show_sources=True, **retrieval_kwargs):
    ...
    reply_content, _ = chat_ollama_response(RAG_SYSTEM_PROMPT, user_prompt)
...
PATIENTS_PATH = "/content/ckm_pts_all_staged.json"   # upload via the Files pane
...
def triage_message(patient, message, use_ehr=True, use_rag=True, use_ckm_stage=False):
    ...
    if use_rag:
        ...
        if use_ehr:
            ...
            parts += patient.get("medications") or []
...
print("\nSummary:", json.dumps(summarize(results_full, "EHR + RAG + CKM Staging Not Known"), indent=2))
...
# Make sure basic differences can be found
conditions = [
...
# Now test with and without patien't CKM stage known
conditions = [
#    ("free text only", False, False, False),
#    ("RAG only", False, True, False),
#    ("EHR only", True,  False, False),
...
# json_files = sorted(glob.glob('/content/*.json'))

# combined_corpus = None
...
#         if combined_corpus is None:
#             # Keep the entire structure from the first file
#             combined_corpus = data
#             if "patients" not in combined_corpus:
#                 combined_corpus["patients"] = []
#         else:
#             # Append only the patients from subsequent files
#             if "patients" in data:
#                 combined_corpus["patients"].extend(data["patients"])

# print(f"\nSuccessfully combined {len(json_files)} files.")
# print(f"Total patients in combined corpus: {len(combined_corpus['patients'])}")

# # Optionally, you can save the combined corpus to a new file:
# with open('/content/ckm_pts_all_staged.json', 'w') as f:
#        json.dump(combined_corpus, f, indent=2)
